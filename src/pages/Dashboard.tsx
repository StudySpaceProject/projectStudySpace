...
const Dashboard = () => {
  ...
  const [topics, setTopics] = useState<Topic[]>([]); // lista sincronizada con TopicsManager
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      setTopics(data.dashboard?.topics || []);
    };
    fetchDashboard();
  }, []);

  // Map topics para cálculo de estadísticas
  const studyTopics: Topic[] = topics.map((topic: any) => ({
    id: topic.id,
  ...
  const filteredTopics = studyTopics.filter(
    (topic: Topic) =>
